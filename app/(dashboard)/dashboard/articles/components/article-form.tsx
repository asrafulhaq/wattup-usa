...
import { ChevronLeft, Image as ImageIcon, Loader2, Pin, Upload } from 'lucide-react';
...
type FormAction =
    | { type: 'SET_FIELD'; field: keyof Omit<FormState, 'isSaving' | 'isUploading'>; value: string | boolean }
...
function formReducer(state: FormState, action: FormAction): FormState {
    switch (action.type) {
        ...
        case 'SET_UPLOADING': return { ...state, isUploading: action.value };
        case 'RESET': return { ...state, ...action.payload, isSaving: false, isUploading: false };
...
function StatusBadge({ status }: { status: string }) {
    ...
    return (
        <Badge variant='outline' className={`text-[11px] font-medium px-2 py-0.5 ${map[status] ?? map['Draft']}`}>
...
export default function ArticleForm({ initialData }: ArticleFormProps) {
    ...
    const [state, dispatch] = useReducer(formReducer, undefined, () => makeInitialState(initialData));
    const { title, slug, content, status, pinned, imageUrl, imageAlt, isSaving, isUploading } = state;

    const set = (field: keyof Omit<FormState, 'isSaving' | 'isUploading'>) =>
        (value: string | boolean) => dispatch({ type: 'SET_FIELD', field, value });
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (res.success && res.data) {
                dispatch({ type: 'SET_FIELD', field: 'imageUrl', value: res.data.url });
                toast.success('Image uploaded successfully');
            } else toast.error('Upload failed');
        } catch { toast.error('Something went wrong during upload'); }
        finally {
    ...
    const handleSave = async (publish = false) => {
        ...
        try {
            const res = await upsertArticle(initialData?.id, {
                title, slug, content, status: publish ? 'Published' : status, pinned, image: imageUrl, imageAlt,
            });
            if (res.success) {
                toast.success(initialData ? 'Article updated' : 'Article created');
                if (initialData) {
                    if (publish) dispatch({ type: 'SET_FIELD', field: 'status', value: 'Published' });
                } else {
                    dispatch({ type: 'RESET', payload: makeInitialState(null) });
                ...
                <div className='flex items-center gap-2 min-w-0'>
                    ...
                    <div className='min-w-0'>
                        <div className='flex items-center gap-2 flex-wrap'>
                            <PageTitle title={initialData ? 'Edit Article' : 'Write Article'} />
                            {initialData && <StatusBadge status={status} />}
                        </div>
                        <p className='text-xs text-muted-foreground mt-0.5'>
                            {initialData ? 'Update your story and republish.' : 'Draft your next story or insight.'}
                        </p>
                    </div>
                </div>
                <div className='flex items-center gap-2 shrink-0'>
                    <Button variant='outline' size='sm' onClick={() => handleSave(false)} disabled={isSaving} className='gap-2 h-9 px-4'>
                        {isSaving ? <Loader2 className='h-4 w-4 animate-spin' /> : <IconDeviceFloppy size={18} />}
                        <span className='hidden sm:inline'>{isSaving ? 'Saving…' : 'Save Draft'}</span>
                        <span className='sm:hidden'>Draft</span>
                    </Button>
                    <Button size='sm' onClick={() => handleSave(true)} disabled={isSaving} className='gap-2 h-9 px-4'>
                        {isSaving ? <Loader2 className='h-4 w-4 animate-spin' /> : <IconSend size={18} />}
            ...
            <div className='flex flex-col gap-4'>
                    ...
                    <CardContent className='p-3'>
                        <div className='flex flex-wrap items-center gap-3'>
                            ...
                            <div className='flex items-center gap-2 flex-1 min-w-[200px]'>
                                <Label htmlFor='slug' className='text-xs text-muted-foreground whitespace-nowrap'>
                                    Slug
                                </Label>
                                <Input
                                    id='slug'
                            ...
                            <div className='flex items-center gap-2'>
                                <Label htmlFor='status' className='text-xs text-muted-foreground whitespace-nowrap'>Status</Label>
                                <Select value={status} onValueChange={set('status')}>
                                    <SelectTrigger id='status' className='h-8 w-32 text-xs'>
                            ...
                            <div className='flex items-center gap-2 border border-border/60 rounded-md px-2.5 py-1.5 bg-muted/20'>
                                <Pin className={`h-3.5 w-3.5 ${pinned ? 'text-primary' : 'text-muted-foreground'}`} />
                                <span className='text-xs font-medium text-muted-foreground'>Pin to Home</span>
                                <Switch checked={pinned} onCheckedChange={set('pinned')} className='scale-75' />
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className='shadow-none border-border rounded-md'>
                    <CardContent className='p-4 sm:p-6 space-y-5'>
                        ...
                        <div className='space-y-1.5'>
                            ...
                            <RichTextEditor value={content} onChange={set('content')} placeholder='Start writing your story…' />
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className='shadow-none border-border rounded-md'>
                    <CardHeader className='px-4 pt-4 pb-2'>
                        <CardTitle className='text-sm font-semibold text-foreground uppercase tracking-wider'>Featured Image</CardTitle>
                    </CardHeader>
                        ...
                        <div className='grid gap-6 md:grid-cols-[1fr_2fr]'>
                            ...
                            <button
                                ...
                                className={`relative w-full aspect-video rounded-md border-2 border-dashed transition-all overflow-hidden flex flex-col items-center justify-center group ${
                                    imageUrl ? 'border-transparent' : 'border-border hover:border-primary/50 hover:bg-primary/5'
                                }`}>
                                {imageUrl ? (
                                    <>
                                        <Image src={imageUrl} alt={imageAlt || 'Featured'} fill className='object-cover' unoptimized />
                                    ...
                                    <div className='flex flex-col items-center gap-2 text-muted-foreground/60 group-hover:text-primary/70 transition-colors'>
                                        ...
                                        <span className='text-[10px] uppercase tracking-tighter'>PNG, JPG, WEBP</span>
                                    </div>
                                )}
                            </button>
                            ...
                            <div className='space-y-4'>
                                <div className='space-y-1.5'>
                                    <Label htmlFor='image-url' className='text-xs font-medium text-muted-foreground uppercase tracking-wide'>Image URL</Label>
                                    <Input
                                        id='image-url'
                                        placeholder='https://...'
                                        value={imageUrl}
                                ...
                                <div className='space-y-1.5'>
                                    <Label htmlFor='image-alt' className='text-xs font-medium text-muted-foreground uppercase tracking-wide'>Alt Text (SEO)</Label>
                                    <Input
                                        id='image-alt'
                                ...
                                {imageUrl && (
                                    <Button
                                        ...
                                        onClick={() => { set('imageUrl')(''); set('imageAlt')(''); }}>
