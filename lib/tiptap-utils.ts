...
export const handleImageUpload = async (
    ...
    // Simulate smooth progress (0 → 90%) while the real upload runs in parallel.
    // The ticker increments every 200ms, slowing down as it approaches 90%
    // so it never "finishes" before the real upload does.
    let simulatedProgress = 0;
    let tickerStopped = false;

    const runTicker = async () => {
        while (!tickerStopped && simulatedProgress < 90) {
            if (abortSignal?.aborted) break;
            // Increment: fast at start, slow near 90
            const remaining = 90 - simulatedProgress;
            const step = Math.max(1, Math.floor(remaining * 0.12));
            simulatedProgress = Math.min(90, simulatedProgress + step);
            onProgress?.({ progress: simulatedProgress });
            await new Promise(resolve => setTimeout(resolve, 200));
        }
    };

    // Start ticker and real upload concurrently
    const tickerPromise = runTicker();

    try {
        const result = await uploadSingleImage(file, { folder: 'articles' });
