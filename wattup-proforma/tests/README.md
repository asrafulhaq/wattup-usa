...
| File | Pins | Checklist |
|---|---|---|
| `gate/request-code.test.ts` | member and non-member get the same bytes, and nothing address-dependent runs before the response; the send happens in `after()`, for a member only; the limiter's hit order | 5.11, 5.5, 5.7a, 2.9, 5.8 |
| `gate/verify-code.test.ts` | every failure (Better Auth's codes, malformed input, the IP limit, a member removed mid-flow) is one 400 with one body; the re-check reads the just-issued session past the cookie cache and signs it out again; the code stays a string | 5.11, 2.19 to 2.21, 2.23, 2.29 |
