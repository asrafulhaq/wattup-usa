 ...
 * are ever exposed is a separate, later decision, and it lives in one place:
 * lib/locations/public.ts. Nothing here decides what the browser sees.
 *
 * Because this file therefore holds private data, it is server side only. See the
 * guard at the top of the generated module.
 *
...
//
// Never import this from a "use client" module. Server components read it and pass the
// projection from ./public down as props. What is public is decided in ./public, not
...
if (typeof window !== 'undefined') {
  throw new Error(
    'lib/locations/data.ts is server only: it holds private landlord data. ' +
      'Import PublicStation from lib/locations/public instead.',
  );
}

export const STATIONS: StationRecord[] = ${JSON.stringify(sites, null, 2)};
`;
