...
function SlidesCard({ card }: { card: SlidesCardData }) {
    return (
        <div className='relative group rounded-[8px] overflow-hidden h-[520px] md:h-[489px] w-full flex flex-col items-start justify-end gap-[10px] p-[32px] pt-[276px] md:pt-[276px]'>
            ...
            <Link
                href={card.cta.href}
                className='relative  z-20 inline-flex px-[24px] py-[12px] bg-primary hover:bg-primary-hover duration-500 text-white  justify-center rounded-[8px] font-bold text-[16px] shadow-btn transition-colors  mt-2 leading-[130%] '>
                {card.cta.label}
            </Link>
        </div>
    );
